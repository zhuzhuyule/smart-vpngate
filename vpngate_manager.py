...
def get_session_token(password: str, username: str = "admin") -> str:
    salt = "smart_vpngate_secure_salt_2026"
        ...
        if killed_pids:
            ...
            for pid in killed_pids:
                try:
                    ...
                    pass
            print(f"[Cleanup] Terminated Smart VPNGate OpenVPN processes: {killed_pids}", flush=True)
    except Exception as e:
...
<head>
  ...
  <title>Smart VPNGate - 安全登录</title>
...
<body>
  <div class="login-container">
    <div class="login-card">
      ...
      <h2 class="login-title">Smart VPNGate</h2>
...
<head>
  ...
  <title>Smart VPNGate 节点池管理系统</title>
  <style>
  ...
  <div class="btn-group">
    ...
    <div class="dropdown">
      ...
      <div id="github_dropdown" class="dropdown-content">
        <a href="https://github.com/zhuzhuyule/smart-vpngate" target="_blank">正式版</a>
        <a href="https://github.com/zhuzhuyule/smart-vpngate/tree/smart" target="_blank">测试版</a>
      </div>
    </div>
  ...
  if (currentPageNodes.length === 0) {
    ...
    $("rows").innerHTML=currentPageNodes.map(n=>{
      ...
      if (isCurrentlyActive) {
        ...
        connectBtn = `<select class="connect-btn" ${_dis} style="width:92px;${_dis ? 'opacity:0.3;cursor:not-allowed;' : ''}" onchange="onSwitchSelect(this, '${esc(n.id)}')"><option value="">切换到…</option>${_opts}</select>`;
      } else {
...
function refreshButtonIdle() {
  ...
  if (!btn) return;
  btn.disabled = false;
  btn.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" style="width:16px; height:16px; flex:0 0 auto;" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182m0-4.991v4.99" /></svg>更新节点`;
}
